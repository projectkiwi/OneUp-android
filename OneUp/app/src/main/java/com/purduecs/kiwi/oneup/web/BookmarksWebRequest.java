 ...
 */
public class BookmarksWebRequest implements OneUpWebRequest<JSONObject, ArrayList<Challenge>> {

    // TODO: this is literally the same as ChallengesWebRequest except I changed the request
    // TODO: url. I feel like that has to be a problem. Also changed parameters.

    Request mRequest;

    public BookmarksWebRequest(int start, int length, final RequestHandler<ArrayList<Challenge>> handler) {
        ...
        mRequest = new JsonObjectEditHeaderRequest(Request.Method.GET, OneUpWebRequest.BASE_URL + "/bookmarks", headerArgs, null,
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        handler.onSuccess(parseResponse(response));
